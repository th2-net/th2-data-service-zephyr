...
class TestZephyrServiceImpl {
    ...
    internal fun `correctly handle handshake`() {
        val responseObserver: StreamObserver<DataServiceInfo> = mock { }
        service.crawlerConnect(crawlerInfo, responseObserver)
        inOrder(responseObserver) {
            verify(responseObserver).onNext(eq(DataServiceInfo.newBuilder().setName("test").setVersion("1").build()))
    ...
    internal fun `returns correct last id on another handshake for same crawler`() {
        testScope.runBlockingTest {
            ...
            service.crawlerConnect(crawlerInfo, responseObserver)
            inOrder(responseObserver) {
                verify(responseObserver).onNext(eq(DataServiceInfo.newBuilder().setName("test").setVersion("1").setEventId(eventData.eventId).build()))
                verify(responseObserver).onCompleted()
                verifyNoMoreInteractions()
            }
        }
